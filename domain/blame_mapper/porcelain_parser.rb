  ...
  module Blame
    ...
    module Porcelain
      ...
      def self.parse_file_blame(output)
        Porcelain.split_by_line_porcelain(output)
                 .map { |line| Porcelain.parse_line_blame(line) }
      end
      ...
      def self.parse_line_blame(porcelain)
        line_block = porcelain.split(NEWLINE)
        line_report = {
          'line_num' => parse_first_porcelain(line_block[0]),
          'code' => line_block[-1]
        }

        line_block[1..-2].each do |line|
          parsed = parse_key_value_porcelain(line)
